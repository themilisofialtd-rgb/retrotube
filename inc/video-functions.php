...
if ( ! function_exists( 'wpst_entry_blog_footer' ) ) {
	function wpst_entry_blog_footer() {
		...
		if ( 'blog' === get_post_type() ) {
			...
			if ( is_array( $blog_cats ) || is_array( $blog_tags ) ) {
				echo '<div class="tags-list">';
				if ( is_array( $blog_cats ) ) {
					foreach ( $blog_cats as $blog_cat ) {
						echo '<a href="' . esc_attr( get_category_link( $blog_cat->term_id ) ) . '" class="label" title="' . esc_attr( $blog_cat->name ) . '"><i class="fa fa-folder-open"></i>' . esc_html( $blog_cat->name ) . '</a> ';
					}
				}
				if ( is_array( $blog_tags ) ) {
					foreach ( $blog_tags as $blog_tag ) {
						echo '<a href="' . esc_attr( get_tag_link( $blog_tag->term_id ) ) . '" class="label" title="' . esc_attr( $blog_tag->name ) . '"><i class="fa fa-tag"></i>' . esc_html( $blog_tag->name ) . '</a> ';
					}
				}
				echo '</div>';
			}
