...
/**
 * Get script to validate HTML code.
 */
	...
	<script>
		;(function() {
			function validateHTML(htmlString) {
				var htmlString = escapeAmpersands( removeStyleTags( removeScriptTags(htmlString) ) );
				try {
					var parser = new DOMParser();
					var doc = parser.parseFromString('<root>' + htmlString + '</root>', "application/xml");
					var parseError = doc.getElementsByTagName("parsererror");
					if ( parseError.length === 0 ) {
						return '';
					}

					return extractParseErrorMessageFromParserErrorElement(parseError[0]);
				} catch(e) {
					return '';
				}
			}

			function extractParseErrorMessageFromParserErrorElement(parserErrorElement) {
				if (!parserErrorElement) return null;

				const temp = document.createElement('div');
				temp.innerHTML = parserErrorElement.innerHTML;

				const errorDiv = temp.querySelector('div');
				if (errorDiv && errorDiv.textContent) {
					return errorDiv.textContent.trim();
				}

				return parserErrorElement.textContent?.trim() || null;
			}

			function extractLineAndColumnFromErrorMessage(errorMessage) {
				const regex = /line (\d+) at column (\d+)/;
			...
			function escapeAmpersands(html) {
				return html.replace(/&(?![a-zA-Z]+;|#[0-9]+;)/g, '&amp;');
			}
			...
			function updateDom(textAreaElt) {
				var cursorPos = getCaretPosition(textAreaElt);
				var eltId = textAreaElt.id || Math.floor(Math.random() * 1000000);
				var messageId = "validate-html-message-" + eltId;
				var htmlError = validateHTML(textAreaElt.value);
				var htmlCodeIsEmpty = ! textAreaElt.value;
				var color = htmlError === '' ? 'green' : 'red';
				var validMessage = '✓ This HTML code is valid.';
				var invalidMessage = '✗ This HTML code is invalid'
					+ '<br>- '
					+ '<span style="font-weight: normal;">'
					+ htmlError
					+ '</span>'
					+ '<br><span style="font-weight: normal; opacity: 0.8">(Cursor position: Line: '
					+ cursorPos.line + ', Column: ' + cursorPos.column + ')</span>';

				var messageElt = updateMessageElt(
					messageId,
					htmlError === '' ? validMessage : invalidMessage,
					color,
					htmlCodeIsEmpty
				);
			...
			function getCaretPosition(textarea) {
				...
				const textUptoCaret = textarea.value.substring(0, pos);
			...
			document.addEventListener("DOMContentLoaded", function(event) {
				document.querySelectorAll('.validate-html-code').forEach(function (textAreaElt) {
					...
					textAreaElt.addEventListener('mouseup', function(e) {
						updateDom(e.target);
					});
				});
			});
